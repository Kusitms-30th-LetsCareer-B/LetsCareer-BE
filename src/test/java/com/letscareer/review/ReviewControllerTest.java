...
public class ReviewControllerTest extends ControllerTestConfig {
    ...
    public void testGetReviewsByRecruitmentId() throws Exception {
        ...
        ReviewResponse.ReviewDto documentReview = new ReviewResponse.ReviewDto(
                1L, "서류 리뷰", "SATISFIED", List.of("지원동기", "논리적 구성"),
                List.of("명확한 표현"), "잘한 점 설명", "개선할 점 설명", null
        );

        ReviewResponse.ReviewDto interviewReview = new ReviewResponse.ReviewDto(
                2L, "면접 리뷰", "AVERAGE", List.of("직무 적합성", "경험 활용"),
                List.of("산업 이해도"), "면접 잘한 점", "면접 개선할 점", null
        );

        ReviewResponse.ReviewDto etcReview1 = new ReviewResponse.ReviewDto(
                3L, "기타 리뷰 1", null, null, null,
                "기타 잘한 점 1", "기타 개선할 점 1", "HIGH"
        );

        ReviewResponse.ReviewDto etcReview2 = new ReviewResponse.ReviewDto(
                4L, "기타 리뷰 2", null, null, null,
                "기타 잘한 점 2", "기타 개선할 점 2", "MEDIUM"
        );
                        ...
                        resource(
                                ResourceSnippetParameters.builder()
                                        ...
                                        .responseFields(
                                                ...
                                                fieldWithPath("data.document.reviewName").description("서류 리뷰 이름"),
                                                fieldWithPath("data.document.satisfaction").description("서류 만족도").optional(),
                                                fieldWithPath("data.document.wellDonePoints").description("서류 잘한 점").optional(),
                                                fieldWithPath("data.document.shortcomingPoints").description("서류 개선할 점").optional(),
                                                fieldWithPath("data.document.wellDoneMemo").description("서류 잘한 점 설명").optional(),
                                                fieldWithPath("data.document.shortcomingMemo").description("서류 개선할 점 설명").optional(),
                                                fieldWithPath("data.document.difficulty").description("서류 난이도 (해당 없음)").optional().type("String").ignored(), // 서류에는 난이도 없음

                                                fieldWithPath("data.interview.id").description("면접 리뷰 ID"),
                                                fieldWithPath("data.interview.reviewName").description("면접 리뷰 이름"),
                                                fieldWithPath("data.interview.satisfaction").description("면접 만족도").optional(),
                                                fieldWithPath("data.interview.wellDonePoints").description("면접 잘한 점").optional(),
                                                fieldWithPath("data.interview.shortcomingPoints").description("면접 개선할 점").optional(),
                                                fieldWithPath("data.interview.wellDoneMemo").description("면접 잘한 점 설명").optional(),
                                                fieldWithPath("data.interview.shortcomingMemo").description("면접 개선할 점 설명").optional(),
                                                fieldWithPath("data.interview.difficulty").description("면접 난이도 (해당 없음)").optional().type("String").ignored(), // 면접에도 난이도 없음
                                                ...
                                                fieldWithPath("data.etc[].satisfaction").description("기타 만족도 (null 가능)").optional(),
                                                fieldWithPath("data.etc[].wellDonePoints").description("기타 잘한 점 (null 가능)").optional(),
                                                fieldWithPath("data.etc[].shortcomingPoints").description("기타 개선할 점 (null 가능)").optional(),
                                                fieldWithPath("data.etc[].wellDoneMemo").description("기타 잘한 점 설명").optional(),
                                                fieldWithPath("data.etc[].shortcomingMemo").description("기타 개선할 점 설명").optional()
                                        )
                        ...
                        resource(
                                ResourceSnippetParameters.builder()
                                        ...
                                        .requestFields(
                                                ...
                                                fieldWithPath("document.reviewName").description("서류 리뷰 이름"),
                                                ...
                                                fieldWithPath("interview.reviewName").description("면접 리뷰 이름"),
